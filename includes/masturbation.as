...
public function masturbateMenu():void {
	...
	if(player.tentacleCocks() > 0 && !assholeOffLimits()) {
...
public function masturbateGo():void {
	...
	//MULTICOCK
		...
		if(player.cockTotal() == 2) {
			...
			if(player.hasSheath()) {
				outputText("Your fingers trace around your sheath, drawing involuntary spasms from your twin members.  ", false);
			}
			//Prep bit + variance based on thickness
			outputText("You grasp one of your " + multiCockDescriptLight() + " in each hand, ", false);
			if(player.averageCockThickness() <= 1.8) outputText("wrapping your fingers around each tool and just holding yourself, the feelings of two cocks almost too much to bear.  ", false);
			else outputText("squeezing as many of your tools as your fingers can encircle.  The feeling of squeezing your thick twin cocks is amazing and overwhelming at the same time.  ", false);
			//Get into it
			//Play with it if <= 10"
			if(player.averageCockLength() <= 10) outputText("Your hips twitch of their own volition, force-fucking your hands.  In moments you're fiercely masturbating,  pumping each shaft in turn, pleasure mounting at the base and growing more urgent with each pump.  You squeal softly, and increase the tempo of your masturbation, desperate for release.  ", false);
			//Tittyfuck if D cup+
			if(player.averageCockLength() > 10 && player.averageCockLength() < 20) {
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				outputText("You look down at your trembling members, mere inches away, and tentatively lick at a cocktip.  Both dicks begin oozing pre in response.  ", false);
				//Try to stick it in a titty!
				...
				{
					outputText("You wrap your " + player.breastCup(0) + " tits around your " + multiCockDescriptLight() + ", sandwiching them inside your pillowy mounds.  With delicious slowness you begin jacking yourself off with your tits, your shafts rubbing together between your luxurious breasts, tongue running figure eights across the tips. ", false);
					if(player.biggestLactation() > 0) outputText("Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ",false);
				}
				//Titfuck (enough breasts for both!)
				if(player.canTitFuck() && player.mostBreastsPerRow() > 2 && !nippleFuck) 
				{
					outputText("You wrap your " + player.breastCup(0) + " tits around your " + multiCockDescriptLight() + ", squeezing them between your " + player.totalBreasts() + " breasts.  With delicious slowness you begin jacking yourself off with your tits, smooth breastflesh massaging your members as you aggressively lick and suck at the tips.  ", false);
					if(player.biggestLactation() > 0) outputText("Jets of milk squirt out of your many nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ",false);
				}
				//No tits, jerk and lick!
				if(player.biggestTitSize() <= 3 && !nippleFuck) 
				{
					outputText("You slowly stroke yourself, pausing every few strokes to smear the streaks of pre that leak from your " + multiCockDescriptLight() + " over yourself.  As you become more and more lubed and horny, you increase the tempo, until at last you are pumping yourself hard and fast, squeezing your cocks together in pleasure.  ", false);
				}
			}
			//Deepthroat yourself, maybe titfuck if equipped.
			if(player.averageCockLength() >= 20) {
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				outputText("Your " + multiCockDescriptLight() + " brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ", false);
				//Try to stick it in a titty!
				if(player.hasFuckableNipples()) 
				{
					//check for dogcocks
					if(player.dogCocks() > 0)
					{
						...
						{ 
							...
							autofellatio = true;
							if(player.normalCocks() > 0) titFuckSingle();
							if(player.horseCocks() > 0) horseTitFuckSingle();
						}  //if you only have dogcocks that are too big, no nipple fuck! Otherwise, run normal multiTitFuck
					} else
					{
						nippleFuck = true;
				...
				if(player.canTitFuck() && !nippleFuck) 
				{
					outputText("You wrap your " + player.breastCup(0) + " tits around your " + multiCockDescriptLight() + ", sandwiching them inside your pillowy mounds.  With delicious slowness you begin jacking yourself off with your tits, your shafts rubbing together between your luxurious breasts, mouthfucking yourself with each of your cocks in turn. ", false);
					if(player.biggestLactation() > 0) outputText("Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ",false);
				}
				//Titfuck (enough breasts for both!)
				if(player.mostBreastsPerRow() > 2 && player.biggestTitSize() > 3 && !nippleFuck) 
				{
					outputText("You wrap your " + player.breastCup(0) + " tits around your " + multiCockDescriptLight() + ", squeezing them between your " + player.totalBreasts() + " breasts.  With delicious slowness you begin jacking yourself off with your tits, smooth breastflesh massaging your members as you aggressively mouthfuck each of your cocks. ", false);
					if(player.biggestLactation() > 0) outputText("Jets of milk squirt out of your many nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ",false);
				}
				//No tits, jerk and lick!
				if(player.biggestTitSize() <= 3 && !nippleFuck) 
		...
		if(player.cockTotal() >= 3) {
			...
			//Play with sheath if has one
			if(player.hasSheath()) {
				outputText("Your fingers trace around your sheath, drawing involuntary spasms from your cocks.  ", false);
			}
			//Prep bit + variance based on thickness
			outputText("You grasp one of your " + multiCockDescriptLight() + " in each hand, ", false);
			...
			if(player.averageCockLength() > 10 && player.averageCockLength() < 20) {
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				outputText("You look down at your trembling members, mere inches away, and tentatively lick at a cocktip.  Your dicks begin oozing pre in response.  ", false);
				//Try to stick it in a titty!
				if(player.hasFuckableNipples()) 
				{
					//check for dogcocks
				...
				if(player.canTitFuck() && !nippleFuck) 
				{
					outputText("You wrap your " + player.breastCup(0) + " tits around your " + multiCockDescriptLight() + ", sandwiching them inside your pillowy mounds.  With delicious slowness you begin jacking yourself off with your tits, your shafts rubbing together between your luxurious breasts, your tongue running figure eights across the tips. ", false);
					if(player.biggestLactation() > 0) outputText("Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ",false);
				}
				//Titfuck (enough breasts for both!)
				if(player.mostBreastsPerRow() > 2 && player.biggestTitSize() > 3 && !nippleFuck) 
			...
			if(player.averageCockLength() >= 20) {
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				outputText("Your " + multiCockDescriptLight() + " brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it in as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ", false);
				//Try to stick it in a titty!
				if(player.hasFuckableNipples()) 
				{
					//check for dogcocks
	...
	if(player.vaginas.length > 0) {
		...
		//Single cunt
		if(player.vaginas.length == 1) {
			outputText("You let your hand roam over your pussy-lips, slowly teasing yourself, diving deeper into your folds to arouse and expose your clit.  ", false);
			//Smaller clits
			if(player.clitLength < 1.5) {
				outputText("You stroke and tease around the sensitive little pleasure bud, letting your fingers plumb the depths below.  ", false);
			}
			//Big clits
	...
	if(player.cockTotal() > 0) {
		...
		if(player.cockTotal() == 1) {
			...
			if(player.lib >= 55 && player.lib <= 80) {
				outputText("As you close your eyes and relax, your " + cockDescript(0) + " surges back to erectness, ensuring ", false);
				if(player.cor < 50) outputText("your dreams will be filled with sex.", false);
				else outputText("you dream in a depraved kinky fantasia.", false);
			}
		...
		if(player.cockTotal() > 1) {
			...
			if(player.lib >= 55 && player.lib <= 80) {
				outputText("As you close your eyes and relax, your dicks surge back to erectness, ensuring ", false);
				if(player.cor < 50) outputText("your dreams will be filled with sex.", false);
				else outputText("you dream in a depraved kinky fantasia.", false);
			}
	...
	//No cock ending
		...
		//Girl ending
		if(player.vaginas.length > 0) outputText("You sigh softly and drift off into a quick nap, smelling of sex.", false);
		//Genderless ending
		else {
...
public function titForeplay()  : void
{
	...
	{
		if(player.lib < 45) outputText("You caress your " + breastDescript(0) + " gently with your fingers", false);
		if(player.lib >= 45 && player.lib < 70) outputText("You grope your " + breastDescript(0) + " agressively with both hands", false);
		if(player.lib >= 70) outputText("You squeeze your " + breastDescript(0) + " brutally with both hands", false);
	}
	else if(player.breastRows.length > 1 && player.biggestTitSize() > 3) {
		if(player.lib < 45) outputText("You hesitantly run your hands across your front, caressing each of your " + num2Text(player.totalBreasts()) + " breasts in turn. ", false);
		if(player.lib >= 45 && player.lib < 70) outputText("You run your hands across your front, squeezing each of your " + num2Text(player.totalBreasts()) + " breasts in turn. ", false);
		if(player.lib >= 70) outputText("You aggressively run your hands across your front, groping each of your " + num2Text(player.totalBreasts()) + " breasts in turn. ", false);
		//different attitude based on player.corruption (ashamed/feels too good to care/reveling in how fucked up you are)
		if(player.cor <= 25) outputText("You blink back tears of shame as you experience the tactile proof of your abnormal endowments, unable to resist mashing your " + allBreastsDescript() + " together", false);
		if(player.cor > 25 && player.cor < 75) outputText("Though a small part of you is horrified by your abnormal endowments, it doesn't stop you from mashing your " + allBreastsDescript() + " together", false);
		if(player.cor >= 75) outputText("The tactile reminder of your abnormal endowments gives you a delicious thrill.  You sigh and mash your tits together", false);
	}
	//If nips are fuckable
	if(player.hasFuckableNipples() && player.biggestTitSize() > 3)
	{
		//more than one row?
		if(player.breastRows.length >= 1) outputText(" as you move your attention to your nipples", false);
		//Different attitude based on libido
		if(player.lib < 60) outputText(". Hesitantly, you rub your fingers across the ", false);
		if(player.lib >= 60) outputText(". Eagerly, you begin to roughly stroke the ", false);
		//Different description based on vag looseness
		if(player.averageVaginalLooseness() < 2) outputText("small, delicate openings at the tip of each nipple. ", false);
		if(player.averageVaginalLooseness() >= 2 && player.averageVaginalLooseness() < 4) outputText("puckered holes at the tip of each nipple. ", false);
		//add "many if lots of nips
		if(player.averageVaginalLooseness() >= 4 && player.averageNipplesPerBreast() > 1) outputText("many ", false);
		if(player.averageVaginalLooseness() >= 4) outputText("swollen, cunt-like lips that top each breast. ", false);
		//Different noise based on sensitivity
		if(player.sens < 45) outputText("You sigh contentedly as ", false);
		if(player.sens >= 45 && player.sens < 70) outputText("You moan lewdly as ", false);
		if(player.sens >= 70) outputText("You squeal with pleasure as ", false);
		//Different description based on wetness
		if(player.averageVaginalWetness() < 2) outputText("you feel your nipples loosen up. ", false);
		if(player.averageVaginalWetness() >= 2 && player.averageVaginalWetness() < 4) outputText("you begin to slowly spread the slippery secretions leaking from your engorged nipples all over your " + breastDescript(0) + ". ", false);
		if(player.averageVaginalWetness() >= 4)
		{
			outputText("little spurts of ", false);
			//Lactating?
			if(player.averageLactation() > 0) outputText(" milky ", false);
			outputText("girlcum squirt out of your engorged nipples. ", false);
		}
		//Special extras for lactation
		if(player.averageLactation() > 0 && player.averageLactation() < 2) outputText("Your freakish mammaries drip a sticky combination of cunt-juice and milk, spattering your " + player.legs() + " and crotch.", false);
		if(player.averageLactation() >= 2 && player.averageLactation() < 3) outputText("Your freakish mammaries leak thin streams of sticky girlcum mixed with milk all over your " + player.legs() + " and crotch.", false);
		if(player.averageLactation() >= 3) outputText("Your freakish mammaries constantly drool a steady stream of milky cunt-juice, drenching your " + player.legs() + " and crotch in milky goo.", false);
	}
	//If nipples aren't fuckable.
	else if(player.breastRows.length > 0 && player.biggestTitSize() > 3) {
		outputText(", emitting ", false);
		//different noises depending on sensitivity
		if(player.sens < 45) outputText("soft gasps of pleasure each time you flick one of your ", false);
		if(player.sens >= 45 && player.sens < 70) outputText("loud moans of pleasure each time you flick one of your ", false);
		if(player.sens >= 70) outputText("squeals of pleasure each time you flick one of your ", false);
		//extra bit if you have LOTS of nipples
		if(player.totalNipples() > 2) outputText("many ", false);
		//different description based on size of nips
		if(player.lust >= 50) {
			if(player.biggestLactation() > 1) {
				if(player.biggestLactation() > 2) outputText("huge, swollen nipples. ", false);
				else outputText("fat, puckered nipples. ", false);
			}
			else outputText("erect nipples. ", false);
		}
		else outputText("nipples.  ", false);
		//Special extras if lactating
		if(player.biggestLactation() > 1 && player.biggestLactation() < 2) outputText("Droplets of milk dribble from each nipple, spattering milk onto your " + player.legs() + " and crotch.  ", false);
		if(player.biggestLactation() >= 2 && player.biggestLactation() < 3) outputText("Thin squirts of milk spray from each nipple, spattering milk onto your " + player.legs() + " and crotch.  ", false);
		if(player.biggestLactation() >= 3) outputText("A constant stream of milk drizzles from each teat, soaking your " + player.legs() + " and crotch.  ", false);
	}
}
...
public function titFuckSingle():void
{
	...
	if(player.averageLactation() == 0)
	{
		...
		if(player.averageVaginalWetness() >= 2 && player.averageVaginalWetness() < 4) {
			...
			else if(player.demonCocks() > 0) outputText("Slick juices dribble down shiny purplish skin and nodules of your " + cockDescript(0) + " and y", false);
			...
			else outputText("Slick juices dribbled down the skin of your " + cockDescript(0) + " and y", false);
		}
	...
	if(Math.round(player.cockArea(0)) >= player.vaginalCapacity())
	{
		...
		if(player.sens < 45) outputText("ou grunt with exertion as you attempt to stuff your " + cockDescript(0) + " into ", false);
		if(player.sens >= 45 && player.sens < 70) outputText("ou blink back tears as you attempt to stuff your "  + cockDescript(0) + " into ", false);
		if(player.sens >= 70) outputText("ou scream with a combination of pain and pleasure as you attempt to stuff your "  + cockDescript(0) + " into ", false);
		//Different based on looseness
		if(player.averageVaginalLooseness() < 2) outputText("the small, over-stretched opening of your swollen nipple. ", false);
		if(player.averageVaginalLooseness() >= 2 && player.averageVaginalLooseness() < 4) outputText("the engorged and distended opening of your fat, swollen nipple. ", false);
		if(player.averageVaginalLooseness() >= 4) outputText("the gaping fuck-mouth of your inhuman nipple-cunt. ", false);
	...
	{
		//Different noises based on sensitivity
		if(player.sens < 45) outputText("ou sigh with pleasure as your " + cockDescript(0) + " slides into ", false);
		if(player.sens >= 45 && player.sens < 70) outputText("ou moan with pleasure as your "  + cockDescript(0) + " slides into ", false);
		if(player.sens >= 70) outputText("ou scream with delight as your "  + cockDescript(0) + " slides into ", false);
		//Different based on looseness
		if(player.averageVaginalLooseness() < 2) outputText("the small, over-stretched opening of your swollen nipple. Your " + cockDescript(0) + " penetrates your swollen nipple easily, sliding half way in on your first thrust. ", false);
		...
		if(player.averageVaginalLooseness() >= 4) {
			...
			else if(player.tentacleCocks() > 0) outputText("the gaping fuck-mouth of your inhuman nipple-cunt. The swollen lips of your bloated nipple engulf the tip of your " + cockDescript(0) + " and begin to slide down its length, swallowing it completely as it twists and pulses on it's own, fucking your nipple.  ", false);
		...
		if(player.averageVaginalWetness() < 2) outputText("girl-lube quickly coats the whole length of your " + cockDescript(0) + " in a glistening layer of fuck juice. " , false);
		if(player.averageVaginalWetness() >= 2 && player.averageVaginalWetness() < 4) outputText("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, pooling at your crotch. ", false);
		if(player.averageVaginalWetness() >= 4) {
			if(player.normalCocks() > 0) outputText("girl-lube pours out of your swollen nipple over your " + cockDescript(0) + " and hands, pooling on the ground below you. ", false);
			else if(player.hasSheath() > 0) outputText("girl-lube drizzles down the length of your " + cockDescript(0) + " in thick streams, pooling in and around your sheath. ", false);
			else if(player.tentacleCocks() > 0) outputText("girl-lube pours out of your swollen nipple over your " + cockDescript(0) + " and hands, pooling on the ground below you. ", false);
			else if(player.demonCocks() > 0) outputText("girl-lube pours out of your swollen nipple over your " + cockDescript(0) + " and hands, pooling on the ground below you. ", false);
			else outputText("girl-lube pours out of your swollen nipple over your " + cockDescript(0) + " and hands, pooling on the ground below you. ", false);
		}		
	}
}
...
{			
	...
	if(holeTotal >= fittableCocks)
	{
		...
		if(player.cocks.length == 2) outputText("Quickly, you move your other cock into position against ",false);
		//more than two dicks but only two that fit, or if you have only two holes anyway
		if(player.cocks.length > 2 && (fittableCocks == 2 || holeTotal == 2)) outputText("Quickly, you move another cock into position against ",false);
		//If you have more than two dicks to use, and enough nipples for all of them
		if(fittableCocks > 2 && holeTotal >= fittableCocks) outputText("Quickly, you move your remaining shafts into position against your other nipples. ",false);
		//How many nips?
		if(holeTotal > 2 && fittableCocks <= 2) outputText("one of your other nipples. ",false);
		if(holeTotal == 2) outputText("your other nipple. ",false);
	}
	//if you dont have enough nipples for all your dicks
	if(fittableCocks > 2 && holeTotal < fittableCocks) 
	{
		outputText("You are momentarily stumped as you realize that you don't have enough nipples for all of your cocks. ", false);
		//can you fit more than one in each?
		if(player.vaginalCapacity() >= player.cockArea(randomCock)*2)
		{
	...
	{
		if(player.averageVaginalWetness() < 2) outputText("Y", false);
		if(player.averageVaginalWetness() >= 2 && player.averageVaginalWetness() < 4) outputText("Slick juices dribble down your shafts, and y", false);
		if(player.averageVaginalWetness() >= 4) outputText("Slick juices squirt out from around your shafts and y", false);
	} 
	else 
	{
		if(player.averageLactation() < 2) outputText("Rivulets of milky girlcum drizzle down your shafts and y", false);
		if(player.averageLactation() >= 2) outputText("Milky girlcum squirts out from around your shafts, staining them white. Y", false);
	}
	//Round and compare cock thickness to vag looseness
	if(Math.round(player.cockArea(randomCock)) >= player.vaginalCapacity())
	{
		//Different noises based on sensitivity
		if(player.sens < 45) outputText("ou grunt with exertion as you attempt to stuff your cocks into ", false);
		if(player.sens >= 45 && player.sens < 70) outputText("ou blink back tears as you attempt to stuff your cocks into ", false);
		if(player.sens >= 70) outputText("ou scream with a combination of pain and pleasure as you attempt to stuff your cocks into ", false);
		//Different based on looseness
		if(player.averageVaginalLooseness() < 2) outputText("the small, over-stretched openings of your swollen nipples. ", false);
		if(player.averageVaginalLooseness() >= 2 && player.averageVaginalLooseness() < 4) outputText("the engorged and distended openings of your fat, swollen nipples. ", false);
		if(player.averageVaginalLooseness() >= 4) outputText("the gaping fuck-mouths of your inhuman nipple-cunts. ", false);
		//Compare cockthickness and vaglooseness more specifically
		//if it barely fits
		if(Math.round(player.cockArea(randomCock)) == player.vaginalCapacity())
		{
			if(player.averageVaginalLooseness() < 2) outputText("Your poor, tortured nipples are barely up to the task of accepting your swollen rods, but accept them they do. ", false);
			if(player.averageVaginalLooseness() >= 2 && player.averageVaginalLooseness() < 4) outputText("The puckered openings that cap your engorged nipples are stretched to their limits as you shove your fat rods home. ", false);
			if(player.averageVaginalLooseness() >= 4) outputText("The swollen lips of your bloated nipples stretch around your throbbing cocks, swallowing them whole. ", false);
			outputText("With each thrust, you bury your shafts deeper into your greedy tits. Overwhelmed by the combined sensations, your mind is barely able to cope with the intense feelings of fullness where no such feelings should be possible. ", false);
		}
		//if it doesnt really fit well
		if(Math.round(player.cockArea(randomCock)) > player.vaginalCapacity())
		{
			if(player.averageVaginalLooseness() < 2) outputText("Your poor, tortured nipples are woefully insufficient compared to the width of your swollen rods. Delirious with arousal, you keep pushing them in anyway. ", false);
			if(player.averageVaginalLooseness() >= 2 && player.averageVaginalLooseness() < 4) outputText("The puckered openings that cap your engorged nipples are stretched to their limits and beyond as you shove your fat rods home. ", false);
			if(player.averageVaginalLooseness() >= 4) outputText("The swollen lips of your bloated nipples gape wide, but the enormous girth of your throbbing members spread them even wider. ", false);
		...
		if(player.averageLactation() > 0) outputText("Milky ", false); 
		else outputText("Slick ", false);
		if(player.averageVaginalWetness() < 2) outputText("girl-lube drizzles down your shafts, coating them in glistening layers of fuck juice. " , false);
		if(player.averageVaginalWetness() >= 2 && player.averageVaginalWetness() < 4) outputText("girl-lube drools from your tortured nipples in thick streams, pooling beneath your ass. ", false);
		if(player.averageVaginalWetness() >= 4) outputText("girl-lube spurts out of your tortured nipples with each thrust of your hips, spattering your arms and face with your secretions. ", false);
	}
	if(Math.round(player.cockArea(randomCock)) < player.vaginalCapacity())
	{
		if(!doubleUp)
		{
			//Different noises based on sensitivity
			if(player.sens < 45) outputText("ou sigh with pleasure as your stiff cocks slide into ", false);
			if(player.sens >= 45 && player.sens < 70) outputText("ou moan with pleasure as your stiff cocks slide into ", false);
			if(player.sens >= 70) outputText("ou scream with delight as your stiff cocks slide into ", false);
			//Different based on looseness
			if(player.averageVaginalLooseness() < 2) outputText("the small, over-stretched openings on your swollen nipples. Your hard rods penetrate your engorged nipples easily, sliding half way in on your first thrust. ", false);
			if(player.averageVaginalLooseness() >= 2 && player.averageVaginalLooseness() < 4) outputText("the engorged and distended openings on your fat, swollen nipples. Your hard rods plunge deeply into your freakishly engorged nipples, penetrating them easily. ", false);
			if(player.averageVaginalLooseness() >= 4) outputText("the gaping fuck-mouths of your inhuman nipple-cunts. The swollen lips of your bloated nipples engulf your hard rods and begin to slide down their length, swallowing them completely. ", false);
			outputText("You revel in the sensation as you slowly stroke your shafts in and out of your distended nipples. Your cocks are enveloped in the warm, wet embrace of your freakish tits, and ", false);
			if(player.averageLactation() > 0) outputText("milky ", false);
			else outputText("slippery ", false);
			if(player.averageVaginalWetness() < 2) outputText("girl-lube drizzles down your shafts, coating them in glistening layers of fuck juice. " , false);
			if(player.averageVaginalWetness() >= 2 && player.averageVaginalWetness() < 4) outputText("girl-lube drools from your fat nipples in thick streams, pooling beneath your ass. ", false);
			if(player.averageVaginalWetness() >= 4) outputText("girl-lube pours out of your swollen nipples in a constant deluge of sticky fluid, creating a large puddle beneath you. ", false);
		} else
		{
			//Different noises based on sensitivity
			if(player.sens < 45) outputText("ou sigh with pleasure as your stiff cocks slide into ", false);
			...
			if(Math.round(player.averageCockThickness())*2 < player.averageVaginalLooseness()/2)
			{
				if(player.averageVaginalLooseness() < 2) outputText("the over-stretched openings on your swollen nipples. Though they are reletively small and delicate, you can easily slide two of your tiny cocks into each of their slippery passages.", false);
				if(player.averageVaginalLooseness() >= 2 && player.averageVaginalLooseness() < 4) outputText("the engorged and distended openings on your fat, swollen nipples. Stretching easily, they can engulf two cocks each with surprising ease, stretching around your turgid rods with little difficulty. ", false);
				if(player.averageVaginalLooseness() >= 4) outputText("the gaping fuck-mouths of your inhuman nipple-cunts. The swollen lips of your bloated nipples squish open as they swallow your fat dicks with little difficulty. ", false);
				outputText("With each thrust, you bury your shafts deeper into your greedy tits. The feeling of your cocks rubbing together inside the warm, wet embrace of your inhuman breasts pushes you towards the edge of insanity, and you are briefly unable to comprehend the world around you. ", false);
			} else	//otherwise, its a tight fit
			{
				if(player.averageVaginalLooseness() < 2) outputText("the small, over-stretched openings on your swollen nipples. Your poor, tortured nipples are stretched painfully as you cram two of your throbbing cocks into one. ", false);
				if(player.averageVaginalLooseness() >= 2 && player.averageVaginalLooseness() < 4) outputText("the engorged and distended openings on your fat, swollen nipples. They are stretched to their limits as you shove two of your fat dicks into one. ", false);
				if(player.averageVaginalLooseness() >= 4) outputText("the gaping fuck-mouths of your inhuman nipple-cunts. The swollen lips of your bloated nipples are stretched to their limits as you push two of your fat dicks into one. ", false);
...
public function multiNippleFuckPrep(randomCock:Number):void {
	...
	if(player.cocks[randomCock].cockType == CockTypesEnum.HUMAN)
	{
		...
		//more than one?
		if(player.normalCocks() > 1) outputText("one of your " + num2Text(player.normalCocks()) + " " + cockDescript(randomCock) + "s ", false);
	...
	if(player.lib < 70) outputText("against the opening of ", false);
	if(player.lib >= 70) outputText("into ", false);
	//More than one row?
	if(player.breastRows.length > 1 && player.averageNipplesPerBreast() > 1) outputText("one of the ",false);
	else if(player.breastRows.length > 1) outputText("the ", false);
	if(player.breastRows.length == 1) outputText("one of your ",false);
	//More than 1 nip per boob?
	if(player.averageNipplesPerBreast() > 1) outputText("many ", false);
	//Different based on looseness (again)
	if(player.averageVaginalLooseness() < 2) outputText("painfully stretched nipple", false);
	if(player.averageVaginalLooseness() >= 2 && player.averageVaginalLooseness() < 4) outputText("freakishly swollen nipple", false);
	if(player.averageVaginalLooseness() >= 4) outputText("huge, bloated cunt-nipple", false);
	if(player.breastRows.length == 1 || player.averageNipplesPerBreast() > 1) outputText("s", false);
	if(player.breastRows.length > 1) outputText(" on one of your lower breasts",false);
	outputText(". ", false);
}
...
public function orgazmo(selfSucking:Boolean, nippleFuck:Boolean):void {
	...
	if(player.cockTotal() > 0) 
	{
		if(player.cockTotal() == 1) {
			...
			else if(player.anemoneCocks() == 1) outputText("base.  The tentacles surrounding your " + cockDescript(0) + " go nuts, constricting around it, inadvertently firing aphrodisiacs into the orgasmic flesh as they wring a potent, hip-jerking climax from you.  ", false);
			else outputText("twitching manhood, growing rapidly.  You stroke furiously, feeling the pressure of your cum as it nears release.  ", false);
			outputText("Pleasurable spasms overwhelm you as cum erupts from your " + cockDescript(0), false);
			outputText(".  Your hips jerk in the air in time with your eruptions, spraying cum ", false);
			if(selfSucking) outputText("into your eager mouth.  ", false);
			if(nippleFuck) outputText("deep inside your unnatural breast. ", false);
			else if(!selfSucking) outputText("in the air.  ", false);
			if(selfSucking) {
				if(player.cumQ() < 25) outputText("You manage to gulp down most of your cum as it bursts from your loins, the pleasure of orgasm goading you into compulsively swallowing.  ", false);
				...
				if(player.cumQ() >= 25 && player.cumQ() < 250) outputText("Cum dribbles down the length of your shaft as your abused tit is filled to overflowing by spurt after spurt of gooey sperm. ", false);
				if(player.cumQ() >= 250 && player.cumQ() < 500) outputText("Pulse after pulse of cum splash deep within your breast, causing it to swell with pressure. Cum runs down your cock in a torrent to pool on you as your orgasm drags on.  ", false);
				if(player.cumQ() >= 500) outputText("Your orgasm never seems to end, and your world dissolves into the feelings from your " + cockDescript(0) + " as it erupts jet after jet of cum into your abused tit.  With a *pop*, your cock is forced free by the pressure. It continues to squirt semen everywhere, while a river of cum pours out of your dialated nipple, pooling around you.  ", false);
			}
			else {
				if(player.cumQ() < 25) outputText("A few thick spurts of cum burst from your loins, splattering you liberally.  ", false);
		...
		if(player.cockTotal() > 1) {
			...
			if(nippleFuck) 
			{
				if(player.cumQ() < 25) outputText("You feel multiple thick spurts of cum splash inside of your breasts. As you buck and thrust with orgasm, your cocks begin to pop free of your abused tits. ", false);
				if(player.cumQ() >= 25 && player.cumQ() < 250) outputText("Cum dribbles down your shafts as your abused tits are filled to overflowing by spurt after spurt of gooey sperm. As you buck and thrust with orgasm, your cocks begin to pop free of your abused tits. ", false);
				if(player.cumQ() >= 250 && player.cumQ() < 500) outputText("Pulse after pulse of cum splash deep within your breasts, causing them to swell with pressure. Torrents of cum run down your cocks to pool around you. As you buck and thrust with orgasm, your cocks begin to pop free of your abused tits.  ", false);
				if(player.cumQ() >= 500) outputText("With an explosive release, you finally cum.  Your orgasm never seems to end, and your world dissolves into the feelings from your " + multiCockDescriptLight() + " as they erupt jet after jet of cum into your abused tits.  With a series of wet *pops*, your cocks are forced free by the pressure. They continue to squirt semen everywhere, while rivers of cum pour out of your dilated nipples, pooling around you.  ", false);
			}

			//Doing the perverted sprinkler after dicks have popped out.
			outputText("Pleasure overwhelms your fragile mind as cum erupts from your " + multiCockDescriptLight() + ".  Your hips jerk in the air in time with your eruptions, cum spraying from you as your body does its best impression of a perverted sprinkler.  ", false);
			if(selfSucking) {
	...
	if(player.vaginas.length > 0) {
		...
		if(player.vaginas.length == 1) 
		{
			...
			if(player.gender == 3) outputText("fem", false);
			outputText("sex quivers as the pleasure overwhelms you, robbing you of muscle control, your passage rippling and contracting around your fingers.  ", false);
			//Big clitty bonus!
...
public function titCum(cumQuantity:Number = 3):void
{
	...
	if(player.averageLactation() >=2 && !player.hasFuckableNipples())
	{
		outputText("As orgasm after orgasm thunders through your body, you feel pressure build in your breasts and then suddenly release, as ", false);
		if(player.averageLactation() <= 2.6) outputText("thin streams of milk spray out of your ", false);
		if(player.averageLactation() > 2.6 && player.averageLactation() < 3) outputText("thick gouts of milk spurt out of your ", false);
		if(player.averageLactation() >= 3) outputText("huge torrents of milk blast out of your ", false);
		//different description based on size of nips
	...
	if(player.hasFuckableNipples())
	{
		outputText("A strange feeling builds in your breasts as your nipples seem to tighten and quiver with anticipation. You ", false);
		//Different noise based on sensitivity
		if(player.sens < 70) outputText("moan with ecstacy as your freakishly swollen nipples ", false);
		if(player.sens >= 70) outputText("scream with ecstacy as your freakishly swollen nipples ", false);
		//Different description based on wetness
		if(player.averageVaginalWetness() < 2) outputText("dribble ", false);
		if(player.averageVaginalWetness() >= 2 && player.averageVaginalWetness() < 4) outputText("spray ", false);
...
public function deluxeDildo():void {
	...
	if(player.hasVagina()) {
		//(highcor)
		if(player.cor > 66) outputText("You retrieve the floppy pink dildo from your possessions and strip down in the middle of your camp, shivering with the sexual thrill of your exhibitionism.", false);
		//(medcor)
		else if(player.cor > 33) outputText("You retrieve your floppy dildo and sigh happily as you squeeze it, feeling the spongy surface give a little bit.  Glancing around furtively, you find a secluded spot and strip down.", false);
		//(lowcor)
		else outputText("You blush feverishly as you grab your pink dildo.  It flops about lewdly as you run off behind some rocks and strip down to use it.  You feel like such a pervert.", false);
		outputText("\n\n", false);
		//(low cor)
		if(player.cor < 50) outputText("You hold the fake dong away from you, aroused but still somewhat disgusted by its lewd shape.", false);
		//high cor
		else outputText("You hold the fake dong, squeezing it and giggling at the realistic texture.  You can't wait to try it out.", false);
		outputText("  A drop of pink aphrodisiac leaks from the tip, offering you a hint of the pleasure you're in for.   You make sure to catch it on your crotch, letting the fluid seep into your " + vaginaDescript(0) + ".  Warmth radiates outwards, spreading to your thighs as your " + clitDescript(), false);
		if(player.clitLength < .5) outputText(" becomes hard and sensitive", false);
		else if(player.clitLength < 3) outputText(" peeks through your folds", false);
		else if(player.clitLength < 6) outputText(" fills with blood, growing erect like a tiny cock", false);
		else outputText(" fills with blood, twitching and pulsating like a man's cock", false);
		outputText(".  You ", false);
		if(player.cor > 50) outputText("don't ", false);
		outputText("hesitate ", false);
		if(player.cor < 50) outputText("before slowly working it inside you, gasping at the enhanced sensitivity of your " + vaginaDescript(0) + ".", false);
		else outputText("ramming it deep inside you, moaning as it rubs your now over-sensitive walls.", false);
		...
		//(Pretty wet)
			...
			if(player.cockTotal() > 0) {
				...
				if(player.totalCocks() == 1) outputText("s", false);
				outputText(", bouncing on your belly.  ", false);
			}
...
public function centaurMasturbation():Boolean {
	...
	//POPULATE THE ARRAY WITH VALUES FOR EACH USABLE SCENE
	//Add fuckable nipples scene
	if(player.hasFuckableNipples()) scene.push(0);
	//Add 'hueg guy' scene
	if(player.hasCock()) {
		if(player.tallness * (5/6) < player.cocks[player.longestCock()].cockLength) scene.push(1);
	}
	//RANDOMLY SELECT A USABLE SCENE
	var selector:Number = scene[rand(scene.length)];
	//NippleCunt scene
	if(selector == 0) {
		...
		outputText("  The openings are soft, sensitive, and slick as you ease a fingertip into ", false);
		if(player.totalNipples() > 2) outputText("two of ", false);
		outputText("them.", false);
		if(player.biggestTitSize() > 2) outputText("  The sudden displacement makes your " + breastDescript(0) + " jiggle enticingly, and the sudden movement fires arcs of pleasure deep into your body.", false);
		else outputText("  They're shallow enough that you don't get much more than a fingertip inside them, but the flesh is extraordinarily sensitive, shooting arcs of pleasure deep into your body.", false);
		outputText("  Your free fingers slowly stroke around the outer edges of your " + nippleDescript(0) + "s' lips, pausing when you feel a tiny sensitive clit-like nub just inside the top of the opening.\n\n", false);
		
		outputText("Shivering with pleasure, you kneel down to prevent your legs from going out from underneath you.   Your " + player.face() + " flushes hotter as you toy with the slippery nipple-cunts, feeling them growing hard and puffy as you turn yourself on more and more.  ", false);
		if(player.biggestLactation() > 1) outputText("Thick, milky lubricant races down the curvature of your breasts as your self-pleasure inadvertently lets your milk down.", false);
		else if(player.wetness() >= 3) outputText("Rivulets of lubricant race down the curvature of your breasts as your chest-pussies clench around your intruding fingers.", false);
		else outputText("Slick lubricant squishes around your fingers as you finger-fuck your chest-pussies.", false);
		...
		if(player.hasVagina() || player.hasCock()) {
			outputText("Denied a single touch, your ", false);
			if(player.hasCock()) {
				if(player.hasVagina()) outputText(multiCockDescriptLight() + " and " + vaginaDescript(0), false);
			...
			if(plural) outputText(" leak their own fluid, quivering with ache and need that you can't reach to satisfy.", false);
			else outputText(" leaks its own fluids, quivering with ache and need that you can't reach to satisfy.", false);
			if(player.totalCocks() > 1) outputText("  You feel each of your " + multiCockDescriptLight() + " tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowments.", false);
			else if(player.totalCocks() == 1) outputText("  You feel your " + multiCockDescriptLight() + " tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowment.", false);
			if(player.balls > 0) outputText("  Rocking slightly inside your sack, your " + ballsDescriptLight() + " repeatedly clench up against your body, ready to release.", false);
			outputText("\n\n", false);
		}
		//(ORGAZMO)
		outputText("A slow wave of contractions starts deep inside each of your " + nippleDescript(0) + "s.  It intensifies as it rises towards the surface of your " + breastDescript(0) + ", reaching a crescendo that brings you to the peak of pleasure.  Your eyes roll back as you slump down in orgasmic bliss, fingers pumping relentlessly at you slippery nipple-holes.  Noisy wet squelches and incessant moaning rouse you from your pleasure-induced coma, eventually waking you back to reality.\n\n", false);
		
		//Optional post orgasm bits for dicks/pussies
		if(player.hasCock()) {
			if(player.cumQ() < 50) outputText("A small puddle of semen has formed under you, ", false);
			else if(player.cumQ() < 200) outputText("A decent sized puddle of white seed has formed under you, ", false);
			else if(player.cumQ() < 1000) outputText("A large puddle of white seed has formed underneath you and even splattered your underside, ", false);
			else if(player.cumQ() < 2000) outputText("A frothing puddle of thick cum has formed below you, soaking your legs and underbelly, ", false);
			else outputText("A small lake of semen surrounds you, ", false);
			outputText("released from your " + multiCockDescriptLight() + " by the intense climax.", false);
			if(player.hasVagina()) outputText("  ", false);
		}
		if(player.hasVagina()) {
			if(player.wetness() < 2) outputText("The scent of pussy hangs in the air, clinging to your moist, puffy cunt-lips.", false);
			else if(player.wetness() < 4) outputText("The air is full of the scent of horny horse-pussy.  If the moist feeling between your hind-legs is any indication, your back-half probably looks and smells like a mare in heat.", false);
			else if(player.wetness() < 5) outputText("The air is saturated with the heady scent of aroused horse-pussy, and if the wetness between your hind-legs is any indication, your hind-quarters would be a slip 'n slide of pleasure for any daring enough to penetrate you.", false);
			else outputText("The air is filled with the thick musk of your bestial horse-pussy.  If the rivulets of moisture dripping down to your hooves are any indication, you might need to get used to the smell.", false);
		}
		if(player.hasCock() || player.hasVagina()) outputText("\n\n", false);
	
		//Real aftermath
		outputText("Judging by the sky, at least an hour has passed.  You sigh and pry your cramped fingers from your aching " + nippleDescript(0) + "s, rubbing the sore entrances with your palm before you climb back up onto your " + player.feet() + ".  As you get dressed, you're very conscious of how much better you feel from the wonderful finger-fuck.  ", false);
		if(player.cor > 66) outputText("You can't wait to do it again.", false);
	...
	//Huge dick jerk
		...
		else if(player.cor < 66) outputText("Sighing in resignation, ", false);
		else outputText("Smiling with excitement, ", false);
		outputText("you blush as ", false);
		if(plural) outputText("each of ", false);
		outputText("your " + multiCockDescriptLight() + " swells out underneath you, hanging down from your equine hindquarters as ", false);
		if(!plural) outputText("it grows full and hard.", false);
		else outputText("they grow full and hard.", false);
		...
		if(player.tallness * 1.5 < player.cocks[player.longestCock()].cockLength) {
			outputText("  In no time flat you're rendered virtually immobile by the length of ", false);
			if(plural) outputText("all ", false);
			outputText("your " + multiCockDescriptLight() + ".", false);
		}
		outputText("\n\n", false);	
		//Get started (not THAT long)
		if(player.tallness * 1.5 >= player.cocks[player.longestCock()].cockLength) {
			outputText("Bending down and grabbing hold with both hands, you take ", false);
			if(plural) outputText("a ", false);
			else outputText("your ", false);
			outputText(cockDescript(primary) + " and lift up, pressing it tightly against your belly as you begin to stroke along the sensitive underside.  ", false);
			if(player.cumQ() > 500) outputText("Thick streams of pre-cum start to drizzle from your tip, forced out due to your prodigious production of sexual fluid.", false);
			else if(player.cumQ() > 100) outputText("A dribble of pre-cum starts to leak from the tip as you get more into the act.", false);
			else outputText("A tiny dollop of pre-cum slowly builds on your tip as you get into the act.", false);
			if(player.hasVagina()) outputText("  Your neglected " + vaginaDescript(0) + " aches for penetration, but there's no way you'll be able to reach it in your current state.", false);
			outputText("  You sigh, delighted that you're able to caress at least one of your sexual organs with this body", false);
			if(player.cor < 40) outputText(", but you're also worried that you're falling into the perversion that lurks in this strange world", false);
			outputText(".  It feels so good that you close your eyes and whinny with delight.\n\n", false);
		
			outputText("You smear your pre-cum over the tip, rolling your palm across the sensitive tip in a way that makes it hard to stand.  Meanwhile your other hand is busy fondling the underside, stroking what little of your length you can reach.  Even with the limited access, you can feel your " + cockDescript(primary) + " pulsing in your grip, growing harder with every touch and caress.", false);
			if(player.totalCocks() == 2) outputText("  Your other dick mimics its lucky brother's pleasure, even though it's been ignored in order for you to focus on your current 'toy'.", false);
			else if(player.totalCocks() > 2) outputText("  Your other " + multiCockDescriptLight() + " mimic their lucky brother's pleasure, even though they've been ignored in order for you to focus on your current 'toy'.", false);
		...
		//Get started (long)
			...
			outputText(cockDescript(primary) + " and lift it up, curving it slightly as you begin to stroke the underside with long, fluid caresses.  You even manage to bend down and give it a lick, forcing you into a pleasurable shudder.", false);
			if(player.cumQ() > 500) outputText("  Thick streams of pre-cum start to drizzle from your tip, forced out due to your prodigious production of sexual fluid.", false);
			else if(player.cumQ() > 100) outputText("  A dribble of pre-cum starts to leak from the tip as you get more into the act.", false);
			else outputText("  A tiny dollop of pre-cum slowly builds on your tip as you get into the act.", false);
			if(player.hasVagina()) outputText("  Your neglected " + vaginaDescript(0) + " aches for penetration, but there's no way you'll be able to reach it in your current state.", false);
			outputText("  You sigh, delighted that you're able to caress your sexual organs with this body", false);
			if(player.cor < 33) outputText(", but you're also worried that you're falling into the perversion that lurks in this strange world", false);
			outputText(".  It feels so good that you close your eyes and whinny with delight as your hands fondle your massive length.\n\n", false);
			
			//STROKE (long)
			outputText("You smear your pre-cum over the tip, rolling your palm across the sensitive tip in a way that makes it hard to stand.  Meanwhile your other hand is making great use of your incredible length, jacking you off with long fluid strokes.  In no time flat, you can feel your " + cockDescript(0) + " pulsing in your grip, growing harder in time with your fevered stroking.", false);
			if(player.totalCocks() == 2) outputText("  Your other dick mimics its lucky brother's pleasure, even though it's been ignored in order for you to focus on your current 'toy'.", false);
			else if(player.totalCocks() > 2) outputText("  Your other " + multiCockDescriptLight() + " mimic their lucky brother's pleasure, even though they've been ignored in order for you to focus on your current 'toy'.", false);
			outputText("  You let your hind legs give out and slump down, locking your forelegs while you tremble with what can only be the approach of an orgasm.\n\n", false);
		}
		//Cumjaculation
		if(player.tallness * 1.5 >= player.cocks[player.longestCock()].cockLength) 
			outputText("Squeezing tightly around your " + cockHead(0), false);
		else outputText("Pumping hard", false);
		outputText(", you orgasm, feeling relief flow through you with every pump of ", false);
		if(player.balls > 0) outputText("your " + ballsDescriptLight(), false);
		else outputText("release", false);
		outputText(".  You arch your back as you blast out your seed, shuddering and spraying it into the air.  It splatters wetly against a rock, oozing down the now-slimy surface ", false);
		if(player.cumQ() < 50) outputText("as you finish unloading against it.", false);
		else if(player.cumQ() < 200) outputText("as you continue to pump more and more seed onto it.", false);
		else if(player.cumQ() < 500) outputText("as you continue to fountain more and more spooge over it.  By the time you finish there's a small puddle at the base and you've thoroughly glazed the rock in question.", false);
		else if(player.cumQ() < 1500) outputText("as you continue to blast it with cum.  Spooge puddles underneath it, slowly spreading while it soaks into the barren ground.", false);
		else {
			outputText("as you continue to erupt cum all over it.  The thick spooge forms a thick puddle at the base, spreading until your hooves are coated in it.", false);
			if(player.cumQ() > 3000) outputText("  As you finish cumming, the puddle deepens.  It's inches thick and splattering everywhere whenever you take a step.", false);
		}
		outputText("  You slump back, still shuddering from the pleasure as you feel your ", false);
		if(!plural) outputText("member", false);
		else outputText("members", false);
		if(player.hasSheath()) {
			if(plural) outputText(" slowly grow limp and slide back inside your sheath.", false);
			else outputText(" slowly growing limp and sliding back inside your sheath.", false);
		}
		else if(minLust() > 40 || player.lib > 50) {
			if(plural) outputText(" grow slightly less hard.  As much as you seem to need sex, that doesn't surprise you.", false);
			else outputText(" growing slightly less hard.  As much as you seem to need sex, that doesn't surprise you.", false);
		}
		else {
			if(plural) outputText(" slowly grow limp, shrinking.", false);
			else outputText(" slowly growing limp, shrinking.", false);
		}
		outputText("\n\n", false);
		//Cunt cums
		if(player.hasVagina()) {
			if(player.wetness() <= 1) outputText("A trickle of wetness oozes down your thighs, filling the air with the scent of centaur sex from your orgasm.", false);
			else if(player.wetness() == 2) outputText("Fluids leak down your thighs, filling the air with the scent of centaur sex as you orgasm.", false);
			else if(player.wetness() == 3) outputText("Thick, clear fluid soaks your thighs, filling the air with the scent of liquid centaur sex as you orgasm.", false);
			else if(player.wetness() == 4) outputText("Thick fluid coats your thighs and drips into a puddle on the ground, filling the air with the pungent aroma of liquid centaur sex as you orgasm.", false);
			else outputText("Spatters of fluids gush out from between your thighs, coating your upper legs in a thick coat of clear feminine lubricant as you orgasm.  The air is filled from the scent of your liquid centaur lust as more and more drips into a rapidly expanding puddle between your rear legs.", false);
			if(player.hasFuckableNipples()) {
				if(player.totalNipples() > 2) outputText("All", false);
				else outputText("Both", false);
				outputText(" of your " + num2Text(player.totalNipples()) + " " + nippleDescript(0) + "s quiver, ", false);
				if(player.wetness() < 4 && player.biggestLactation() < 2) outputText("leaking", false);
				else if(player.wetness() < 5 && player.biggestLactation() < 3) outputText("dripping", false);
				else outputText("squirting", false);
				outputText(" equal quantities of ", false);
				if(player.biggestLactation() < 1) outputText("clear", false);
				else outputText("milk-laced", false);
				outputText(" lubricant.  You pause to finger their entrances, shivering unconsciously as you slowly come down.", false);
			}
			outputText("\n\n", false);
		}
...
public function centaurDudesGetHorseAids():void {
	...
	outputText("Seeing the toy's exposed, gaping genitals, you feel a stirring in your " + multiCockDescriptLight() + ".  You yearn to touch yourself, but your centaur lower body prevents you, as usual.  Grunting with annoyance, you trot up to the toy and give its wide cunt an experimental fisting.  ", false);
	//[If small cock: 
	if(player.cockArea(x) < 30) outputText("Your hand slips in easily... since it's made for real horsecocks, it's a bit too big to give you any satisfaction.  Your gaze shifts upwards to the toy's fake anus, which seems a bit more your size.</i>\"", false);
	else outputText("Your fist slips in easily, and you give the toy a few preparatory thrusts to make sure it's nice and ready for your hefty cock.", false);
	outputText("\n\n", false);
	
	outputText("Satisfied the toy is ready for you, you clop back a few paces and surge forward.  You mount the toy easily, your belly running across its smooth, warm back until your chest bumps against the mare's head.  You grip her shoulders for support and start to buck your hips, your " + cockDescript(x) + " poking around in search of entrance.  Finally, you feel the tip of your prick ", false);
	if(player.cockArea(x) < 30) outputText("pressing against the tight ring of the toy's anus", false);
...
public function getHugeEggsInCawk():void {
	...
	if(player.statusAffectv1(StatusAffects.Exgartuan) == 1 && player.statusAffectv2(StatusAffects.Exgartuan) == 0 && flags[kFLAGS.TIMES_EGGED_IN_COCK] == 0) {
		...
		if(player.cor < 33) outputText("into your bedroll");
		else if(player.cor < 66) outputText("off behind a rock");
		...
		if(player.cor >= 75) outputText("corrupted ");
		...
		if(!player.isTaur()) {
			...
			if(player.balls == 0) outputText("swollen belly");
...
public function birthBeeEggsOutYourWang():void {
	...
	if(player.isTaur()) outputText("y");
		...
		if(player.balls > 0) outputText("them");
		else outputText("it");
	...
	if(player.balls == 0) outputText("stomach");
	...
	if(player.cor < 50) outputText("amused");
...
public function layEggsInYerTits():void {
	...
	if(player.statusAffectv1(StatusAffects.Exgartuan) == 2 && player.statusAffectv2(StatusAffects.Exgartuan) == 0) {
		...
		if(player.hasVagina()) {
			if(player.balls > 0) outputText("  Behind your [balls], y");
			else outputText("Y");
		...
		outputText("\n\nOverwhelmed with a sexual glee, you begin to jackknife, driving your ovipositor from tip to deep inside your breast.  Holding everything in place with one hand, your other begins to toy with a spare [nipple].  [if (cocks > 0)   [EachCock] hardens to the full extent, forcing you to take a moment to rearrange, and leaving you jacking off [oneCock] with your spare hand instead.]");
		if(player.hasVagina()) {
		...
		//Stage Three no/high chance
			...
			if(player.bRows() < 2) outputText("\n\nHaving popped your ovipositor out, you spend a few moments contemplating the pulsating organ, which isn't even moving the eggs without the surety of a nice warm hole.  You feel stuck, trapped on the edge of orgasm with an egg half down the tube.  Finally, your arousal and desperation overcomes your better judgment again, and you open your mouth wide for your egg tube.  Immediately, you feel the til-then backed up lubricant of your ovipositor flowing down your gullet, and your mouth and throat begin to feel strange.");
...
public function tentacleSelfFuck():void {
	...
	else if(player.wetness() <= 3) outputText("wet");
	...
	if(player.balls > 0) outputText("  You lift you [balls] out of the way and");
	else outputText("  You");
	...
	outputText(", twisting your [nipple] as your body goes white-hot with pleasure");
	if(player.hasFuckableNipples()) outputText(", even stuffing a finger inside sloppy, dripping nipplecunt");
	outputText(".");
	...
	if(player.tentacleCocks() >= 3) {
		...
		if(player.cumQ() >= 500) {
			outputText(".  That doesn't really matter for much with how heavy your load is, ");
			if(player.cumQ() < 1000) outputText("and your cheeks with the size of each squirt");
	...
	if(player.tentacleCocks() == 2) {
		...
		if(player.cumQ() < 50) outputText("in small spurts");
		else if(player.cumQ() < 200) outputText("in thick ropes");
	...
	if(player.tentacleCocks() > 1) outputText("s retract");
...
public function gildedCockTurbate():void {
	...
	if(player.cockArea(x) < 6) outputText("  Enveloped in the cocksock, you can barely see the tip of your tiny dick as your head peeks out on every downstroke.  The rounded edge of the metallic sleeve rubs maddeningly against the head of your cock, cool and smooth.");
	//[medium dicks] 
	else if(player.cockArea(x) < 20) outputText("  The head of your cock abuts against the edge of the cocksock, bumping against it with every stroke.  With your thickness, the cocksock is a perfect fit, tugging on your cock with every upstroke, stretching your dick lightly as you jack yourself.");
	//[large dicks]
