...
private function cinnabarNonHugeDickings():void {
	...
	if(player.hasVagina()) {
		if(assholeOffLimits()) {
			if(player.vaginalCapacity() < 30) outputText("fingers plunge", false);
			else outputText("fist plunges", false);
		}
		else {
			if(player.vaginalCapacity() < 30 && player.analCapacity() < 30) outputText("fingers plunge", false);
			else if(player.vaginalCapacity() >= 30 && player.analCapacity() >= 30) outputText("fists plunge", false);
			else outputText("fingers and fist plunge", false);
		}
	}
	else {
		if(player.analCapacity() < 30) outputText("fingers plunge", false);
		else outputText("fist plunges", false);
	}
	outputText(" further inside you, violating you far more gently, but still hard enough to get you off.", false);
	...
	if(player.hasVagina()) {
		outputText("  She licks your " + clitDescript() + ", and your " + vaginaDescript(0) + " convulses around the intruding digits, making you shiver and moan.");
		if(!assholeOffLimits()) {
			outputText("  Not wanting to be left out, your " + assholeDescript() + " clamps and puckers with it, wringing the rat's ", false);
			if(player.vaginalCapacity() < 30) outputText("fingers", false);
...
private function cinnabarGetsFUKKKKED():void {
	...
	outputText("Your " + cockHead(x) + " hits those plush, juice-gushing lips and forces them wide.  She's tight, so goddamned tight, but you've only got the barest portion of your tip inside.  Cinnabar squeaks in pleasure and pain, rocking her ass up and down to try and loosen up her poor, abused entrance.\n\n", false);
...
private function fuckCinnabarHugerAndCorruptedEr():void {
	...
	outputText(", squirting juices from the callous, cunt-piercing thrust.  The " + cockHead(x) + " of your " + cockDescript(x) + " can be seen just below her neck", false);
	...
	else outputText("It's too much for your utterly encapsulated member and violated hole, and your poor prostate practically wrings itself dry as it tries to unload.", false);
...
private function cinnabarHuger():void {
	...
	outputText(" to prod at your " + assholeOffLimits(vaginaDescript(), assholeDescript()) + ", sliding in to push your prostate.  She hits it hard enough to make you pump cum with enough force to distort her already-ballooned body, and you go into an orgasmic frenzy, dragging your " + cockDescript(x) + " back to her entrance before jamming in all ", false);
	...
	outputText("At last, your " + cockHead(x) + " pops from her pussy, showing just how completely you've worked over her body.  Her hips are stretched damn near a foot apart, and you can see deep enough inside her ", false);
...
private function cinnabarHuge():void {
	...
	outputText(".  You rub your hand over her belly, feeling your " + cockHead(x) + " through her distended, dick-dominated fur.  Each fiery, pumping load of cum boils underneath the rat's fur, bubbling between your fingers as it sloshes into her womb.  Cinnabar is thrashing, squeaking and crying, though it's quite clear her tears are tears of joy.  She gasps, \"<i>So big", false);
